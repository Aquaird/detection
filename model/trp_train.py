...
flags.DEFINE_integer(
    "max_epoch", 100, "Max classification epoch."
)
flags.DEFINE_integer(
...
flags.DEFINE_float(
    "keep_prob", 0.8, "the rate of keep prob in dropout"
)
flags.DEFINE_float(
...
def main(_):
    ...
    with g.as_default() ,tf.Session(config=gpuconfig, graph=g) as session:
        ...
        with tf.name_scope("Train"):
            train_input = Input(config, data=trp_reader.raw_data('train', 'all'))
        ...
        with tf.name_scope("Valid"):
            valid_input = Input(config, data=trp_reader.raw_data('valid', 'all'))
        ...
        writer = tf.summary.FileWriter(logdir=FLAGS.save_path)
        writer.add_graph(g)
        ...
        for i in range(config.max_epoch):
            ...
            if valid_perplexity < best_cost:
                ...
                logfile.flush()
                sv.save(session, FLAGS.save_path+'/best_model.ckpt', global_step=i)
